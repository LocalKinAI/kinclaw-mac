...
for f in range(1, len(frames) + 1):
    ...
    cam.location = (radius * math.sin(angle), -radius * math.cos(angle), 1.45)
    cam.keyframe_insert("location", frame=f)
    aim.location = (0, -0.1, h0 + (h1 - h0) * t); aim.keyframe_insert("location", frame=f)

# ---- preview: every frame plainly lit, small — to look at the movement, not to film from
...
scene.render.resolution_x = scene.render.resolution_y = 352
scene.render.image_settings.file_format = "PNG"
scene.frame_step = 4
scene.render.filepath = os.path.join(out, "view-")
